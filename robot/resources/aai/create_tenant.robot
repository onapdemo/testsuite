...
${AAI_SIMPLE_TENANT_BODY}=    robot/assets/templates/aai/create_tenant_body.template
...
Create Tenant Without Relationship
    [Documentation]    Creates a service in A&AI if it doesn't exist
    [Arguments]    ${cloud_owner}  ${cloud_region_id}  ${tenant_id}    ${tenant_name}
    ${data_template}=    OperatingSystem.Get File    ${AAI_SIMPLE_TENANT_BODY}
    ${arguments}=    Create Dictionary     tenant_id=${tenant_id}    tenant_name=${tenant_name}
    ${data}=	Fill JSON Template    ${data_template}    ${arguments}
    ${put_resp}=    Run A&AI Put Request     ${INDEX PATH}${ROOT_TENANT_PATH}${cloud_owner}/${cloud_region_id}/tenants/tenant/${tenant_id}     ${data}
    ${status_string}=    Convert To String    ${put_resp.status_code}
    Should Match Regexp    ${status_string} 	^(201|200)$

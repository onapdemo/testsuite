...
Create VID VNF module
    ...
    ${request_id}=    Parse Request Id     ${response text}
    Capture Page Screenshot    ${vnf_name}.png
    Log    VFModule_Screenshot=${vnf_name}.png
    Run Keyword If    "${AZURE_CLOUD_REGION}"=="${lcp_region}"    Poll MSO Get Request For Azure    ${GLOBAL_MSO_STATUS_PATH}${request_id}   COMPLETE
    ...    ELSE    Poll MSO Get Request    ${GLOBAL_MSO_STATUS_PATH}${request_id}   COMPLETE
    [Return]     ${instance_id}
